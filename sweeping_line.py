...
class SweepingLine(Scene):
    def construct(self):
        ...
        def opacity_updater(obj: VMobject):
            # if round(get_winding_number(growing_circle.get_anchors() - obj.get_center())) > 0:
            if sum((growing_circle.points[0] - growing_circle.get_center())**2) >= sum((obj.get_center() - growing_circle.get_center())**2):
        ...
        def color_updater(obj):
            if np.dot(moving_line.get_start(), moving_line.normal_vector) > np.dot(obj.get_center(), moving_line.normal_vector):
        ...
        for i in range(30):
            p = Dot(fill_opacity=0.6).move_to([random.uniform(-6, 6), random.uniform(-4, 4), 0])
            p.add_updater(opacity_updater)
            self.add(p)
        
        self.play(growing_circle.animate.scale_to_fit_width(1.5*config.frame_width), run_time=5)
